import { DepartmentMemberManager } from "@/components/department-member-manager";
...
export default async function DepartmentMembersPage({ params }: DepartmentMembersPageProps) {
  ...
  return (
    <main style={{ width: "min(1180px, calc(100% - 2rem))", margin: "1rem auto 2rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ marginBottom: 0 }}>{department.name} Members</h1>
          <p style={{ color: "var(--muted)" }}>
            Manage people records, roles, and contact details for this department.
          </p>
        </div>
        <LogoutButton />
      </div>

      <DepartmentMemberManager departmentSlug={slug} departmentName={department.name} />
    </main>
  );
}
