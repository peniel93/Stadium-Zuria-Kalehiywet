...
type DepartmentAnnouncementsPageProps = {
  params: Promise<{
    slug: string;
  }>;
};
...
export default async function DepartmentAnnouncementsPage({ params }: DepartmentAnnouncementsPageProps) {
  ...
  if (!isSupabaseConfigured()) {
    return (
      <main style={{ padding: "2rem", fontFamily: "var(--font-body)" }}>
        <h1>{department.name} Announcements</h1>
  ...
  return (
    <main style={{ width: "min(1180px, calc(100% - 2rem))", margin: "1rem auto 2rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ marginBottom: 0 }}>{department.name} Announcements</h1>
          <p style={{ color: "var(--muted)" }}>
            Department-scoped notices, timings, and visibility controls.
          </p>
        </div>
        <LogoutButton />
      </div>
