...
type DepartmentChildrenPageProps = {
  params: Promise<{ slug: string }>;
};

export default async function DepartmentChildrenPage({ params }: DepartmentChildrenPageProps) {
  ...
  if (!isSupabaseConfigured()) {
    return (
      <main style={{ padding: "2rem", fontFamily: "var(--font-body)" }}>
        <h1>{department.name} Children Profiles</h1>
  ...
  return (
    <main style={{ width: "min(1180px, calc(100% - 2rem))", margin: "1rem auto 2rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ marginBottom: 0 }}>{department.name} Children Profiles</h1>
          <p style={{ color: "var(--muted)" }}>
            Manage children records by age, class, section, and registration details.
          </p>
        </div>
        <LogoutButton />
      </div>

      <ChildrenManager departmentSlug={slug} departmentName={department.name} />
    </main>
  );
}
